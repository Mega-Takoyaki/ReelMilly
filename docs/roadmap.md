...
| Phase 1 | inbox ingest、sidecarマージ、ready/posted移動、**SQLiteへのメタデータ記録**（[ADR-0011](adr/0011-sqlite-state-store.md)）、**NSFW自動仕分け（`nsfw_auto_rating`記録）**、**画像内容説明の取得（`content_description`記録、両方成功で`ready`）**（[ADR-0015](adr/0015-ai-content-description-and-caption-generation.md)） | 実装済み（`src/core/ingest.py`/`nsfw.py`/`analysis.py`/`generation.py`）。NSFW判定精度・内容説明の実データ検証は未実施 |
| Phase 1.5 | 本体UI（一覧・サムネイル表示・フォルダ管理・タグ管理・**投稿承認/`content_rating`確定操作**・**生成AI設定画面**。全文検索は対象外）（[ADR-0010](adr/0010-custom-asset-management-over-eagle.md)、[ADR-0012](adr/0012-primary-ui-with-telegram-as-secondary.md)、[ADR-0015](adr/0015-ai-content-description-and-caption-generation.md)） | 実装済み（`src/core/web/`、Flask） |
...
| Phase 4 | drop/teaser/engageジョブ結合、部分失敗ルール、last_run管理、**`content_rating_confirmed`未承認アセットの投稿対象外フィルタ**、**自動実行スケジューラ**、**投稿文の自動生成（auto/draft）** | dropジョブのFanvue投稿部分（`posting/jobs.py`の`run_fanvue_drop`/`run_fanvue_drop_batch`、`reelmilly run drop --count/--kind/--rating`、成功時の投稿済みタグ付与、投稿文自動生成）と、`config.yaml`の`cadence`設定に基づく自動実行（`reelmilly run-due`/`reelmilly watch`）を実装済み。X紹介投稿・x_teaser・x_engagementはPhase 3待ち |
